function RoofRightRobotOrientation(newxPos, newyPos, newAngle)
...
if(~newStatus~= 0)
    if ((abs(command_X(command_Index) - xPos(end)) <= 0.15) && ...
        ...
        if command_Index < length(command_X)
            ...
            if(~strcmp(currentDisplay,'Reached Right Roof'))
                currentDisplay = 'Reached Right Roof';
                set(f_currentDisplay, 'String', currentDisplay);
            end
            readyToClearRoof = 0; %We arrived in front of the right part of the roof
        end
    end
end
