function  MoveBack(newxPos, newyPos, newAngle)
global currentSubTask roofCleaned Ctask command_X command_Y...
...
if(~strcmp(currentSubTask,'Moving Back'))
    currentSubTask = 'Moving Back';
...
if(~newStatus~= 0)
    if ((abs(command_X(command_Index) - xPos(end)) <= 0.15) && ...
        ...
        if command_Index < length(command_X)
            ...
            if(~strcmp(currentDisplay,'Reached Standby Position'))
                currentDisplay = 'Reached Standby Position';
                set(f_currentDisplay, 'String', currentDisplay);
            end
            roofCleaned = 0; %We arrived in front of the right part of the roof
            Ctask = 0;
        end
    end
end
