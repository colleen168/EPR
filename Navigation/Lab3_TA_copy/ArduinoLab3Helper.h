...
class EncoderMeasurement {
  ...
  private:
    ...
    const float wheelDiameter = (0.090);  //encoder to wheel, in meters
    const float enc2Wheel = PI * wheelDiameter * 1.0 / (76.0 * 26.0); //encoder to wheel, Gearing = 1/26
    const float enc2Theta = 2.0 * PI / 76.0 * 1.0 / 26.0; //convert encoder counts to radians
    const float maxMV = 0.65;//22*15.48*wheelDiameter/2/26;  //22.2 V * 15.48 rad/s/V / (53+1/12) *pi*D
};
...
class RobotPosition {
  public:
    ...
    */
    ...
    void GPSupdate(SerialCommunication & reportData, PathPlanner & pathPlanner); //GPS correction
